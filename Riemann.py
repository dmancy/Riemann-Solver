...
#    W_right.print()
    ...
    if (P_star > W_left.p):
        ...
        S_L = W_left.u - W_left.c * ((gamma+1)/(2*gamma) * (P_star/W_left.p) + (gamma-1)/(2*gamma))**.5
    ...
    if (P_star > W_right.p):
        ...
        S_R = W_right.u + W_right.c * ((gamma+1)/(2*gamma) * (P_star/W_right.p) + (gamma-1)/(2*gamma))**.5
        ...
        Right_Wave = Wave.Expansion_Fan(1, S_TR, S_HR, W_right.c, W_right.u, W_right.p, W_right.rho)
        W_right_star = State("Right star", gamma, rho_star_r, U_star, P_star)
